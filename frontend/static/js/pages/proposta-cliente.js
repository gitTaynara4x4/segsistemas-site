(() => {
  ...
  function tokenFromPath() {
    ...
    const index = parts.indexOf('proposta');
  ...
  function renderContractRegistration(response, scroll = false) {
    ...
    if (complete) $('contract-complete-text').textContent = response.concluido_em ? `Concluído em ${dateTime(response.concluido_em)}. Os dados já foram registrados pela SEG.` : 'Os dados já foram registrados pela SEG.';
  ...
  async function loadContractRegistration(scroll = false) {
    ...
    try {
      const response = await api(`/api/proposta-publica/${encodeURIComponent(state.token)}/cadastro-contrato`);
  ...
  async function completeContractRegistration(event) {
    ...
    try {
      ...
      $('contract-complete-text').textContent = response.mensagem || 'Os dados foram enviados para a SEG.';
  ...
  async function load() {
    ...
    try {
      const response = await api(`/api/proposta-publica/${encodeURIComponent(state.token)}`);
  ...
  async function approve() {
    ...
    try {
      await api(`/api/proposta-publica/${encodeURIComponent(state.token)}/aprovar`, {
  ...
  async function sendChange() {
    ...
    try {
      await api(`/api/proposta-publica/${encodeURIComponent(state.token)}/solicitar-alteracao`, {
